# Prototipos — historial de diseño

Diseño y prototipos del portafolio de **José Manuel Sánchez**, producidos entre el 5 y el 10 de septiembre de 2026,
ordenados cronológicamente. **No se publican**: el sitio real es el proyecto Astro de la raíz del repositorio
([`README`](../README.md)), construido a partir del prototipo final (sección 5).
...
Un solo archivo HTML con rutas por hash (`#/es/proyectos` ⇄ `#/en/work`). El sitio Astro conserva su diseño, sus
animaciones y sus demos, con el mismo marcado (comprobado nodo a nodo), pero con URL reales por idioma.
...
Desde esta carpeta (`prototipos/`), el archivo final se genera concatenando las partes de
[`05-prototipo-final/fuente/`](05-prototipo-final/fuente/):
...
```bash
node 05-prototipo-final/pruebas/pruebas.cjs
```

Sin dependencias. Sale con código 1 si algo falla. Comprueba HTML seguro, que `build.sh` reproduce el archivo,
plantillas y rutas, demos con entradas maliciosas y datos corruptos, fondos vivos, red de seguridad del movimiento,
colisiones de estilos y contraste AA. El sitio Astro tiene su propia batería en `tests/` (raíz del repositorio).

### Fotografías

Las 14 fotos de [`05-prototipo-final/fotos/`](05-prototipo-final/fotos/) son de [Unsplash](https://unsplash.com/license)
(uso libre, sin atribución obligatoria; igualmente se acreditan en las demos). Autores en `fotos/creditos.txt`.
...
## Ver los prototipos en local

Desde la raíz del repositorio:

```bash
python -m http.server 8765 --bind 127.0.0.1 --directory prototipos
```

Luego abre <http://127.0.0.1:8765/05-prototipo-final/prototipo.html>. La misma configuración está en
`.claude/launch.json` con el nombre `prototipos`.
